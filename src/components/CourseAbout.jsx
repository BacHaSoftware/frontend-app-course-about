...
const CourseAboutPage = () => {
  ...
  const url = 'https://lms.digilife.today';
  const appsurl = 'https://apps.lms.digilife.today';
  ...
  useEffect(() => {
    ...
    const match = currUrl.match(regex);
    setCourseId(match[1]);
  }, []);
  ...
  const handleTabClick = (id, sectionClass) => {
    setActive(id);
  ...
  const handleEnroll = async () => {
    if (!authenticatedUser) {
      ...
      const redirection = `/courses/${courseId}/about`;
      window.location.href = `https://apps.lms.digilife.today/authn/login?next=${encodeURIComponent(redirection)}`;
      return;
    }
    ...
    if (courseId) {
      ...
      fetch(`${url}/api/enrollment/v1/enrollment/${encodedCourseId}`, {
        ...
        // If the response text is empty, set enrolled to false and return
          ...
          const courseDdata = JSON.parse(text);

          if (Object.keys(courseDdata).length !== 0) { // Check if the object is not empty
            if (courseDdata.is_active) {
              setEnrolled(true);
  ...
  useEffect(() => {
    if (courseId) {
      const courseUrl = `${getConfig().LMS_BASE_URL}/api/courses/v1/courses/${courseId}`;
      const enrolmentUrl = `${getConfig().LMS_BASE_URL}/api/enrollment/v1/course/${courseId}`;

      const updateData = async () => {
        const [firstResponse, secondResponse] = await Promise.all([
          axios.get(courseUrl),
          axios.get(enrolmentUrl),
        ]);
        ...
        <div className="white-continer-claim">
          ...
          <div className="">
            <div className="course-info-banner">
              ...
              <span className="info-tab">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="bi bi-cup-hot" viewBox="0 0 16 16">
            ...
            <AnimationOnScroll duration={1.5} animateIn="animate__animated animate__fadeInLeft">
              <div className="course-info-banner2">
                <div className="tabs">
                  {tabItems.map(({ id, title, sectionClass }) => (
                    <TabItemComponent
                      key={title}
                      title={title}
                      onItemClicked={() => handleTabClick(id, sectionClass)}
                      isActive={active === id}
                    />
                  ))}
